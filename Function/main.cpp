#include <iostream>
#include<cstdlib>
#include<ctime>


// функц ничего не возвращающая и не принимающая
// выводит текст в консоль
void say_hello() {
	std::cout << " Hello World! \n";
	std::cout << " Bye World! \n";
}
void print_sum(int num1, int num2) {
	std::cout << num1 << " + " << num2 << " = " << num1 + num2 << '\n';
}
// функция, принимвающая два целых числа и возвращающая их сумму
int sum(int num1, int num2) {
	//int tmp = num1 + num2;
	//return tmp;
	return num1 + num2;
}

void print_arr(int arr[],const int length) { // вывод массива в консоль
	std::cout << '[';
	for (int i = 0; i < length; i++)
		std::cout << arr[i] << ", ";
	std::cout << " \b\b]\n";
}

int x2(int num) {
	num *= 2;
	return num;

}
void v_x2(int num) { // num - параметр
	num *= 2;
}

//задача1
void pos_index(int arr[], const int length) {
	for (int i = 0; i < length; i++)
		if (arr[i] > 0)
			std::cout << i << ' ';
	std::cout << std::endl;

}
//з2
int arrays_max(int arr1[], const int length1, int arr2[], const int length2) {
	int max = arr1[0];
	for (int i = 1; i < length1; i++)
		max = arr1[i] > max ? arr1[i] : max;
	for (int i = 0; i < length2; i++)
		max = arr2[i] > max ? arr2[i] : max;
	return max;
}


int main() {
	setlocale(LC_ALL, "Russian");
	int n,m;
	/*
	say_hello();
	std::cout << " .... \n";
	say_hello();
	std::cout << "Введите два числа ";
	std::cin >> n >> m;
	// print_sum(n,m); // вызов фунц. типа void с двумя параметрами
	std::cout<< sum(n, m)<<'\n'; // вызов фун, возвращ 
	
	// функции и массивы
	const int size = 5;
	int arr[size]{ 7,3,10,8,5 };
	std::cout << " Изначальный массив \n";
	print_arr(arr,size);
	arr[2] = 20;
	std::cout << " Итоговый массив \n";
	print_arr(arr, size);
	

	n = 10;
	//std::cout << x2(n) << std::endl; // 20
	v_x2(n); // n- аргумент
	std::cout << n << std::endl; 
	int mass[]{ 5,6,7 };
	//arr_x2(mass);
	v_x2(mass[1]);
	std::cout<<
	*/
	/*
	//задача 1 индексы положительных элементов
	std::cout << " задача1 \n массив: \n";
	const int size1 = 10;
	int arr1[size1];
	srand(time(NULL));
	for (int i = 0; i < size1; i++)
		arr1[i] = rand()%21 - 10;
	print_arr(arr1, size1);
	std::cout << " Индексы положительных элементов \n";
	pos_index(arr1, size1);
	std::cout << '\n';
	*/
	//задача 2 Максимум в двух массивах
	std::cout << " задача2 \n массив1: \n";
	const int size2_1 = 5;
	int arr2_1[size2_1];
	const int size2_2 = 6;
	int arr2_2[size2_2];
	srand(time(NULL));
	for (int i = 0; i < size2_1; i++)
		arr2_1[i] = rand() % 51;

	for (int i = 0; i < size2_2; i++)
		arr2_2[i] = rand() % 51;

	print_arr(arr2_1, size2_1);
	std::cout << " Массив 2 \n";
	print_arr(arr2_2, size2_2);
	std::cout << " Максимум равен " << arrays_max(arr2_1, size2_1, arr2_2, size2_2) << '\n';



	return 0;
}